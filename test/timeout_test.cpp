...
TEST_CASE("Single Call backs") {
  ...
  SECTION("Timer 1000ms expires after 2000ms. ") {
    ...
    Timer<std::chrono::milliseconds> timer(std::chrono::milliseconds(1000),
                                           [&]() { triggered = true; });
  ...
  SECTION("Timer 2000ms, auto stopped ") {
    ...
    Timer<std::chrono::milliseconds> timer(std::chrono::milliseconds(2000),
                                           [&]() { triggered = true; });
    REQUIRE(triggered == false);
    REQUIRE(timer() == false);
...
TEST_CASE("Elapsed") {
  SECTION("1000ms") {
    ...
    l_log->info("Target {}ms +-{}ms, actual: {}", required_time,
                required_margin, ms);
    REQUIRE(ms == target);
  }
}
